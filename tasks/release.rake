  ...
  in_dir(WORKSPACE_DIR) do
    stage('ExtractVersion', 'Extract the last version from CHANGELOG.md and derive next version unless specified', :always_run => true) do
      ...
      unless next_version
        ...
        min_length = version_parts[1].length
        min_length = min_length > 2 ? 2 : min_length
        next_version = "#{version_parts[0]}.#{sprintf("%0#{min_length}d", version_parts[1].to_i + 1)}#{version_parts.length > 2 ? ".#{version_parts[2]}" : ''}"
...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/realityforge/getopt4j/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']}) · [Full Changelog](https://github.com/realityforge/getopt4j/compare/#{from}...v#{ENV['PRODUCT_VERSION']})
HEADER
...
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('realityforge/getopt4j', tag, :name => tag, :body => changes, :draft => false, :prerelease => prerelease)

      candidates = client.list_milestones('realityforge/getopt4j').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('realityforge/getopt4j', milestone[:number], :state => 'closed')
        end
      end
    end
  end
